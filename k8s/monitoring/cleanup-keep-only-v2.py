#!/usr/bin/env python3
"""
Script để chỉ giữ lại 1 dashboard duy nhất:
...
# Dashboard duy nhất cần giữ lại
...
def cleanup_keep_only_v2(auth_token, dry_run=True):
    """Chỉ giữ lại dashboard v2, xóa tất cả khác"""
    ...
    if keep_dashboard:
        ...
        print("   Tất cả dashboard sẽ bị xóa!")
    ...
    if keep_dashboard:
        ...
        print(f"   ✅ {keep_dashboard['title']}")
    else:
        print("⚠️ CẢNH BÁO: Không tìm thấy dashboard v2!")
        print("   Tất cả dashboard sẽ bị xóa!")
    ...
    if keep_dashboard:
        ...
        print("⚠️ Không có dashboard nào được giữ lại!")
...
def main():
    """Hàm chính"""
    print("🧹 Script Chỉ Giữ Lại Dashboard V2")
    print("=" * 50)
    print(f"📋 Dashboard sẽ GIỮ LẠI:")
    print(f"   ✅ {KEEP_DASHBOARD}")
    print("=" * 50)
    ...
    # Dọn dẹp dashboard
    if cleanup_keep_only_v2(auth_token, dry_run):
        if dry_run:
            print("\n✅ Chạy thử hoàn tất!")
            print("💡 Để thực sự xóa dashboard, chạy:")
            print("   python k8s\\monitoring\\cleanup-keep-only-v2.py --execute")
        else:
            print("\n🎉 Dọn dẹp dashboard hoàn tất!")
            print(f"📋 Dashboard còn lại: {KEEP_DASHBOARD}")
